...
export default {
  ...
  methods: {
    submitInfo() {
      ...
      if(!this.currency || !this.weight) return this.$alert('Please select all the missing information','Error','error');
          this.loading=true;
      let info={
       currency:this.currency,
       weight:this.weight,
       mtl:this.$store.getters.website.websitename
      }
api.MetalsQuery(info).then((res)=>{
     const data = res.data;
    this.loading = false;
           this.$fire({
            ...
            if (promiseres.value) {
              this.$router.push({
                ...
                params: {
                  ...
                  image: this.$store.getters.website.websitesrc,
                  weight:data.weight,
                  name:data.name,
    ...
    checkmetal() {
      if (this.$store.getters.website.websitename == "XAG") {
